...
public class JPSPathFinder : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        void Find()
        {
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                stopwatch.Start();
                bool success = JPS.Find(_nodes, _start, _end, _path);
                stopwatch.Stop();

                _text.text = $"{(success ? stopwatch.ElapsedMilliseconds : -1)}";
            }
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        if (_obstacles != null)
        {
            ...
            {
                var bounds = obstacle.GetComponent<Obstacle>().Bounds;

                Gizmos.DrawWireRect(bounds.center, bounds.size, Color.red);
            }
        }
    }
}
